...
if type(benchName) is list:
    ...
    for curBench in benchName:
        ...
        exeStr = binaryName + " " + lefStr + defStr + constStr + " -cpu %d -output_def %s/%s_%s.def | tee %s/%s_%s_out.log" % (
        numThreads, outpos, curBench, curTime, logpos, curBench, curTime)
        print(exeStr)
    ...
    exeStr = binaryName + " " + lefStr + defStr + constStr + " -cpu %d -output_def %s/%s_%s.def | tee %s/%s_%s_out.log" % (
    numThreads, outpos, benchName, curTime, logpos, benchName, curTime)
